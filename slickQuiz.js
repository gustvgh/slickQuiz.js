(function($){
    $.slickQuiz = function(element, options) {
        var plugin   = this,
            ...
            defaults = {
                checkAnswerText:  'Chequear !',
                nextQuestionText: 'Siguiente &raquo;',
                backButtonText: '',
                completeQuizText: '',
                tryAgainText: '',//Intentalo de nuevo
              ...
              scoreLinkShare: '',
                ...
                numberOfQuestions: 30, //default null
            ...
            _quizScoreShare        = _element + ' .quizScoreShare', //agregado 1 - mportillo
            ...
            $quizScoreShare        = $(_quizScoreShare), //agregado 2 - mportillo
        ...
        plugin.method = {
            ...
            setupQuiz: function(options) { // use 'options' object to pass args
                ...
                for (i in questions) {
                    if (questions.hasOwnProperty(i)) {
						...
						var imageQuestion = ''; // mportillo
						
						imageQuestion = question.i;
							
						questionHTML.append(imageQuestion);
            ...
            completeQuiz: function(options) {
                ...
                var displayShare = $(location).attr('href'); // agregado - mportillo

                if (plugin.config.disableScore) {
                    $(_quizScore).remove()
                    $(_quizScoreShare).remove()
                } else {
                    $(_quizScore + ' span').html(plugin.config.scoreTemplateText
                        .replace('%score', displayScore).replace('%total', questionCount));
                    $(_quizScoreShare + ' span').html(plugin.config.scoreLinkShare
                        .replace('%porcentage', displayScore).replace('%urlshare', displayShare));       //agrado 3 - mportillo             
                }
                //$quizName
